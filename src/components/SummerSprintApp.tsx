...
function SpaceFocusView({ space, onBack, onExit, onStartFocus }: { space: Space; onBack: () => void; onExit: () => Promise<void>; onStartFocus: (projectId: number, projectOptions: SpaceFocusProject[]) => Promise<void> }) {
  ...
  return (
    <div className="space-focus-page-clay rounded-[2.5rem] bg-[#fff7ea] p-5 shadow-soft sm:p-8">
      <header className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3 sm:gap-4"><div title={space.name}><p className="text-sm font-black text-orange-700">MyTime</p><h1 className="mt-1 text-2xl font-black tracking-tight sm:text-3xl">我的空间</h1></div>
          <nav className="space-focus-nav inline-flex shrink-0 items-center gap-1 rounded-full bg-white p-1.5 shadow-sm" aria-label="空间导航">
            <button type="button" onClick={onBack} className="space-focus-nav-key flex h-10 w-10 items-center justify-center rounded-full text-slate-400 transition hover:bg-[#fff1d9] hover:text-orange-700" aria-label="我的空间"><svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="m3 10 9-7 9 7v10a1 1 0 0 1-1-1h-5v-6H9v6H4a1 1 0 0 1-1-1V10Z" strokeLinejoin="round" /></svg></button>
            <span className="space-focus-nav-key is-active flex h-10 w-10 items-center justify-center rounded-full bg-[#fff1d9] text-orange-700" aria-label="专注"><svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><circle cx="12" cy="12" r="8.5" /><path d="M12 7v5l3 2" strokeLinecap="round" strokeLinejoin="round" /></svg></span>
          </nav>
        </div>
        <button type="button" onClick={() => void onExit()} className="space-focus-exit-button shrink-0 rounded-full bg-white px-4 py-2.5 text-xs font-black text-slate-500 shadow-sm">退出空间</button>
      </header>
      ...
      {!loading && data ? <>
        <section className="space-focus-main-panel mt-6 rounded-[2rem] bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-start justify-between gap-4"><div><p className="text-sm font-black text-orange-700">本周专注时间</p><h2 className="mt-1 text-xl font-black">{data.weekStart.replaceAll('-', '.')} — {data.weekEnd.replaceAll('-', '.')}</h2></div><button type="button" onClick={() => editing ? void savePlan() : setEditing(true)} disabled={saving} className="space-focus-small-action rounded-full bg-[#fff0d7] px-4 py-2 text-sm font-black text-orange-800 disabled:opacity-60">{editing ? (saving ? '保存中…' : '保存') : '编辑'}</button></div>
          <div className="mt-5 grid grid-cols-[1fr_auto_1fr] items-end gap-3">
            <label className="space-focus-time-card rounded-2xl border border-orange-100 bg-[#fffaf4] px-4 py-3"><span className="block text-xs font-bold text-slate-500">每天开始</span>{editing ? <input type="time" value={startTime} onChange={(event) => setStartTime(event.target.value)} className="mt-1 w-full bg-transparent text-xl font-black outline-none" /> : <strong className="mt-1 block text-xl">{startTime}</strong>}</label>
            <span className="pb-4 text-lg text-slate-400">→</span>
            <label className="space-focus-time-card rounded-2xl border border-orange-100 bg-[#fffaf4] px-4 py-3"><span className="block text-xs font-bold text-slate-500">每天结束</span>{editing ? <input type="time" value={endTime} onChange={(event) => setEndTime(event.target.value)} className="mt-1 w-full bg-transparent text-xl font-black outline-none" /> : <strong className="mt-1 block text-xl">{endTime}</strong>}</label>
          </div>
